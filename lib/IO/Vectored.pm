...
The first advantage of vectored-IO is that it reduces the number of system calls required. This provides an atomicity guarantee in that your reads/writes won't be intermingled with the reads/writes of other processes where you aren't expecting it. Reducing the number of system calls also eliminates a constant performance overhead.

Another advantage of vectored-IO is that doing multiple system calls can sometimes result in excess network packets being sent. The classic example of this is a web-server handling a static file. If the server C<write(2)>s the HTTP headers and then C<write(2)>s the file data, the kernel might send the headers and file in separate network packets. Ensuring a single packet would be better for latency and bandwidth consumption. L<TCP_CORK|http://baus.net/on-tcp_cork/> is a solution to this issue but it is Linux-specific and requires more system calls.
...
The non-standard C<sendfile(2)> system call can do one less copy than vectored-IO because the file data can be copied directly from the filesystem cache into the final network packet (assuming the hardware and network driver support scatter-gather IO).

Another advantage of C<sendfile()> is that the file data is never actually mapped into virtual address space. Because the network hardware can gather the data from the OS file-system cache (and the OS file-system cache can gather from disk) with Direct Memory Access (DMA), there is less pressure on the Translation Lookaside Buffer (TLB). The consequence is less CPU usage per page transferred.

With C<sendfile()>, the number of bytes to send with each system call is specified by C<size_t> so you still have to call it multiple times in order to send files too large to map into virtual memory on 32-bit systems. However, C<sendfile()> doesn't require re-maping large files throughout the send like vectored-IO does so it can send files in the fewest number of system calls on 32-bit systems.

Because of the above reasons, a good rule of thumb is that C<sendfile()> is best for large files and vectored-IO is best for small files.

Unfortunately, where operating systems have implemented it at all, the C<sendfile()> interfaces are horribly non-standard. The rest of this section will briefly describe the pros and cons of some implementations.
...
As well as a Linux-like C<sendfile()>, Solaris has a fully vectorised interface called C<sendfilev()> which allows the arbitrary mixing of files and in-process memory buffers. Although in many ways this is the best of both worlds, it doesn't guarantee atomicity like traditional vectored-IO does. Note that Solaris also provides C<sendfile64()> and C<sendfilev64()> interfaces because C<off_t> can be 32 or 64 bits so an explicitly 64-bit transitional interface is required.

As if all the caveats above weren't enough, many C<sendfile()> implementations will only work when sending from a file (obviously) and to a network socket (less obviously). So in order for your code to be fully general/portable you may have to implement one code path that uses C<sendfile()> and one that doesn't.
