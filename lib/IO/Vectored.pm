...
Although some people consider these interfaces contrary to the minimalist design principles of unix, they provide certain advantages which are described below.

This module is an interface to your system's L<readv(2)|http://pubs.opengroup.org/onlinepubs/009695399/functions/readv.html> and L<writev(2)|http://pubs.opengroup.org/onlinepubs/009695399/functions/writev.html> vectored-IO system calls specified by POSIX.1. It exports the functions C<syswritev> and C<sysreadv> which are almost the same as the C<syswrite> and C<sysread> perl functions except that they accept multiple strings and have implicit length and offset parameters.
...
Like C<sysread>/C<syswrite>, the vectored versions also croak for various reasons such as passing in too many arguments (greater than the C<IO::Vectored::IOV_MAX> constant), trying to use a closed file-handle, or trying to write to a read-only/constant string (see the C<t/exceptions.t> test for a full list).
...
Think about truncating strings like C<sysread> does. Don't depend on the non-truncating behaviour of C<sysreadv> until version 1.000: You have been warned. :)
...
Although C<sendfile()> is less general than -- and not really compatible with -- vectored-IO, here are some perl interfaces:
